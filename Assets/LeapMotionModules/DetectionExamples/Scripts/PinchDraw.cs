...
namespace Leap.Unity.DetectionExamples {
  public class PinchDraw : MonoBehaviour {
    ...
    [SerializeField]
    private int _drawResolution = 4;

    [SerializeField]
    private float _minSegmentLength = 0.01f;

        [SerializeField]
        private GameObject Horse;
        private motionC scriptC;
    ...
    public Color DrawColor {
      ...
      set {
		...
		private Color _destinineColor = Color.red;
		private Color _originalColor = Color.blue;

		public void upDateColor(){
			if (_destinineColor == _drawColor) {
				//random color, move for it
				_originalColor = _drawColor;
				_destinineColor = new Color(Random.Range(0.0f,1.0f), Random.Range(0.0f,1.0f), Random.Range(0.0f,1.0f));

			} else {
				DrawColor += (_destinineColor - _originalColor) / 50;
			}
		}
    ...
    void Start() {
      ...
      for (int i = 0; i < _pinchDetectors.Length; i++) {
        _drawStates[i] = new DrawState(this);
            ...
            scriptC = Horse.GetComponent<motionC>();
            mParticles.SetActive(true);
            mParticles.transform.position = new Vector3(-0.008f, -0.033f, 0.4f);
            _drawStates[0]._prevRing0 = mParticles.transform.position;
    }
    ...
    void Update() {
				...
				if (detector.DidStartHold) {
                    if (_drawStates[0].notInitialize)
                    {
                       // _drawStates[0].BeginNewLine();
                        CountDownTimer.GetComponent<timerCounting>().StartCountDown(5f);
                    }
			
				}
				if (detector.IsHolding || detector.IsPinching ){
					_drawStates[0].UpdateLine (detector.Position);

				}

				if (detector.DidRelease) {
					_drawStates[0].TempStop();
				}
    /* for (int i = 0; i < _pinchDetectors.Length; i++) {
        var detector = _pinchDetectors[i];
    ...
    private class DrawState {
      ...
      private List<Color> _colors = new List<Color>();
			public bool notInitialize = true;
      ...
      public GameObject BeginNewLine() {
        ...
        //GameObject lineObj = new GameObject("Line Object");
				...
				GameObject lineObj = new GameObject ("Line Object");
				lineObj.transform.parent = _parent.mLines.transform;

				_parent.mObject.Add(lineObj);
      ...
      public void FinishLine() {
        ...
        public void TempStop(){
                ...
                _parent.mParticles.transform.position = _prevRing0;

                _parent.mParticles.SetActive(true);
